  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-4">
      <div>
        ...
        <h1 className="text-2xl font-extrabold text-white">Employee & Team Accounts</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full p-3 rounded-xl bg-gray-900 border border-gray-800 text-xs text-white focus:outline-none focus:border-cyan-500 transition"
              required
            />
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-gray-300">Role</label>
            <select
              value={role}
              onChange={(e: any) => setRole(e.target.value)}
              className="w-full p-3 rounded-xl bg-gray-900 border border-gray-800 text-xs text-white focus:outline-none focus:border-cyan-500 transition cursor-pointer"
            >
      ...
      <div className="glass-panel p-6 rounded-3xl space-y-4">
        <h2 className="text-base font-bold text-white">Active System Accounts ({usersList.length})</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {usersList.map((u) => (
            <div key={u.userId} className="p-4 rounded-2xl bg-gray-900/80 border border-gray-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img src={u.avatar} alt={u.name} className="w-10 h-10 rounded-xl object-cover border border-cyan-500/30" />
                <div>
